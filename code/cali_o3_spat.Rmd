...
df <- df %>% rename(C("time"="test.time", "ozone"="test.ozone"))
...
plot_ly(df, x = ~test.time, y = ~predicted, type = "scatter") %>%
  layout(
         xaxis = list(title = 'Date',
                      zerolinecolor = '#ffff',
                      zerolinewidth = 2,
...
plot_ly(full_data, x = ~date, y = ~predicted, type = "scatter") %>%
  add_trace(full_data, x=~date, y = ~ozone) %>%
  layout(
         xaxis = list(title = 'Date',
                      zerolinecolor = '#ffff',
                      zerolinewidth = 2,
