...
```{r all polutants, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r linear model all pol, echo=FALSE, message=FALSE,warning=FALSE}
set.seed(42)
...
plot_ly(df, x = ~test.ozone, y = ~predicted, type = "scatter") %>%
...
```{r xg model, echo=FALSE, message=FALSE,warning=FALSE}
set.seed(42)
...
```{r xgboost plots}
plot_ly(full_data, x = ~ozone, y = ~prediction, type = "scatter") %>%
